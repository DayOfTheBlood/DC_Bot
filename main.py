...
class DraftBoardView(discord.ui.View):
    ...
    async def _ensure_prereqs(self, interaction: discord.Interaction) -> bool:
        cid = self.channel_id
        if not formats[cid] and tb_mode[cid] != "noTB":
            await interaction.response.send_message("Bitte erst **!bo3** oder **!bo5** setzen.", ephemeral=True)
            return False
        if not coinflip_used[cid]:
            await interaction.response.send_message("Bitte zuerst **!coinflip** benutzen.", ephemeral=True)
            return False
        if turns[cid] not in ["A", "B"]:
            await interaction.response.send_message("Bitte **!first** oder **!second** ausführen.", ephemeral=True)
            return False
        return True
    ...
    async def btn_ban(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if not await self._ensure_prereqs(interaction):
            return
        if _next_action(cid) != "ban" and tb_mode.get(cid) != "noTB":
            await interaction.response.send_message("Momentan ist kein BAN an der Reihe.", ephemeral=True)
            return

        opts = _remaining_killers(cid)
        if not opts:
            await interaction.response.send_message("Keine Killer übrig.", ephemeral=True)
            return

        # Select bauen (<=25 Optionen, wir haben <25 Killer)
        class BanSelect(discord.ui.Select):
            def __init__(self, channel_id: int):
                options = [discord.SelectOption(label=k, value=k) for k in opts[:25]]
                super().__init__(placeholder="Killer zum Bannen wählen…", min_values=1, max_values=1, options=options)
                self.channel_id = channel_id
        ...
        v = discord.ui.View(timeout=60)
        v.add_item(BanSelect(cid))
        await interaction.response.send_message("BAN auswählen:", view=v, ephemeral=True)
    ...
    async def btn_pick(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if not await self._ensure_prereqs(interaction):
            return
        if _next_action(cid) != "pick":
            await interaction.response.send_message("Momentan ist kein PICK an der Reihe.", ephemeral=True)
            return
        ...
        class PickSelect(discord.ui.Select):
            def __init__(self, channel_id: int):
                options = [discord.SelectOption(label=k, value=k) for k in opts[:25]]
                super().__init__(placeholder="Killer zum Picken wählen…", min_values=1, max_values=1, options=options)
                self.channel_id = channel_id
        ...
        v = discord.ui.View(timeout=60)
        v.add_item(PickSelect(cid))
        await interaction.response.send_message("PICK auswählen:", view=v, ephemeral=True)

    @discord.ui.button(label="Undo", style=discord.ButtonStyle.secondary)
    async def btn_undo(self, interaction: discord.Interaction, button: discord.ui.Button):
        if not _user_is_staff(interaction.user):
            await interaction.response.send_message("Nur Staff darf Undo.", ephemeral=True)
            return
        async with _lock_for_channel(self.channel_id):
            msg = await _apply_undo(interaction, self.channel_id)
            await _update_or_create_board(interaction.channel, force_existing=True)
...
async def _update_or_create_board(channel: discord.TextChannel, *, force_existing: bool = False):
    """Erstellt oder aktualisiert das Board im Channel."""
    cid = channel.id
    init_channel(cid)  # falls nötig
    emb = _build_board_embed(cid)
    view = DraftBoardView(cid)

    # Buttons dynamisch enablen/disablen
    next_act = _next_action(cid)
    for item in view.children:
        if isinstance(item, discord.ui.Button):
            if item.label == "BAN":
    ...
    if mid:
        try:
            ...
            board_message_id.pop(cid, None)
            # fällt durch zum Neu-Erstellen

    if force_existing:
        # gewünscht war Update, aber es gibt keine Message – dann neu anlegen
        pass
...
@has_any_role(STAFF_ROLES)
async def board_cmd(ctx):
    """Erstellt/aktualisiert das Status-Board in diesem Channel."""
    await _update_or_create_board(ctx.channel)
    await ctx.message.add_reaction("✅")
